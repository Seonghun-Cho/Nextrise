...
def get_ir_download_buttons(page):
    return page.locator("button[onclick*='companyIRFileDownload']")
...
def download_files_on_current_page(page, page_no: str):
    ...
    print(f"[page {page_no}] IR자료 다운로드 버튼 {button_count}개 발견")
    ...
    if button_count == 0:
        ...
        write_log({
            ...
            "status": "no_buttons",
            "file_path": "",
            "error": "IR자료 다운로드 버튼 없음"
        })

        print(f"[page {page_no}] IR자료 다운로드 버튼을 찾지 못했습니다.")
        return
    ...
    for i in range(button_count):
        ...
        for attempt in range(1, DOWNLOAD_RETRY_COUNT + 2):
            try:
                download_buttons = get_ir_download_buttons(page)
                ...
                debug_screenshot(
                    page,
                    f"17_download_timeout_page_{page_no}_{i + 1}_attempt_{attempt}.png"
                )
                ...
                debug_screenshot(
                    page,
                    f"18_download_error_page_{page_no}_{i + 1}_attempt_{attempt}.png"
                )
        ...
        if not success:
            ...
            print(
                f"[page {page_no}] "
                f"{i + 1}번째 다운로드 최종 실패. "
                f"연속 실패 수: {consecutive_failures}"
            )

            if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                debug_screenshot(
                    page,
                    f"19_stop_due_to_consecutive_failures_page_{page_no}.png"
                )
                raise RuntimeError(
                    f"연속 {MAX_CONSECUTIVE_FAILURES}개 다운로드 실패로 중단합니다. "
                    f"마지막 에러: {last_error}"
                )
...
def go_to_target_page(page, target_page_no: int) -> bool:
    """
    검색 결과 페이지에서 target_page_no로 바로 이동합니다.

    1순위: fnCompanyList(targetPageNo) 직접 호출
    2순위: 페이지 번호/오른쪽 화살표를 이용해 이동
    """
    print(f"{target_page_no}페이지로 직접 이동 시도")
    debug_screenshot(page, f"19_before_go_to_target_page_{target_page_no}.png")

    before_first_company_id = get_first_company_row_id(page)

    try:
        print(f"fnCompanyList({target_page_no}) 직접 호출")
        page.evaluate(
            """
            (targetPageNo) => {
                if (typeof window.fnCompanyList === 'function') {
                    window.fnCompanyList(targetPageNo);
                    return true;
                }
                return false;
            }
        ...
        page.wait_for_timeout(5000)
        wait_until_page_changed(page, before_first_company_id, target_page_no)

        active_page = get_current_active_page(page)
        print("직접 호출 후 active 페이지:", active_page)

        debug_screenshot(page, f"20_after_go_to_target_page_{target_page_no}_by_function.png")

        if active_page == str(target_page_no):
            print(f"{target_page_no}페이지 직접 이동 완료")
            return True

        print("fnCompanyList 직접 호출 후 active 페이지 확인 실패. fallback 진행")

    except Exception as e:
        print(f"fnCompanyList({target_page_no}) 직접 호출 실패:", e)
    ...
    while current_page_no < target_page_no:
        ...
        if not moved:
            print(f"{target_page_no}페이지로 이동 중 실패")
            return False

        active_page = get_current_active_page(page)

        try:
            current_page_no = int(active_page)
        except Exception:
            current_page_no += 1

        if current_page_no == target_page_no:
            print(f"{target_page_no}페이지 이동 완료")
            return True

    return get_current_active_page(page) == str(target_page_no)
...
def go_to_next_page(page, current_page_no: int) -> bool:
    ...
    debug_screenshot(page, f"21_before_go_to_page_{next_page_no}.png")
    ...
    try:
        ...
        if next_number_button.count() > 0:
            ...
            if text == str(next_page_no):
                ...
                debug_screenshot(page, f"22_after_go_to_page_{next_page_no}_by_number.png")
    ...
    try:
        ...
        wait_until_page_changed(page, before_first_company_id, next_page_no)
        debug_screenshot(page, f"22_after_go_to_page_{next_page_no}_by_arrow.png")
        ...
        debug_screenshot(page, f"22_go_to_page_{next_page_no}_failed.png")
        print(f"{next_page_no}페이지 이동 실패:", e)
        return False


def download_all_pages_from(page, start_page_no: int):
...
def main():
    ...
    with sync_playwright() as p:
        ...
        if START_PAGE > 1:
            moved_to_start = go_to_target_page(active_page, START_PAGE)

            if not moved_to_start:
                raise RuntimeError(f"{START_PAGE}페이지로 이동하지 못했습니다.")

        print(f"{START_PAGE}페이지부터 전체 다운로드 시작")
        download_all_pages_from(active_page, START_PAGE)
