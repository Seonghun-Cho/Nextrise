...
def get_download_number(page_no: str, item_index_zero_based: int) -> int:
    """
    회사 목록이 역순이므로 파일명 앞에 붙일 번호를 계산합니다.
...
def login(page):
    ...
    try:
        ...
        debug_screenshot(page, "02_login_button_disabled.png")
        raise RuntimeError("입력 후에도 로그인 버튼이 활성화되지 않았습니다.")
...
def download_files_on_current_page(page, page_no: str):
    ...
    max_download_count = button_count

    for i in range(max_download_count):
        ...
        try:
            ...
            print(
                f"[page {page_no}] "
                f"{i + 1}/{max_download_count}번째 다운로드 시도 "
...
def get_first_company_row_id(page):
    try:
        ...
        () => {
            const row = document.querySelector('#search_list_wait tbody tr[id^="company_view_"], #search_list tbody tr[id^="company_view_"]');
            return row ? row.id : '';
        }
        """)
        return row_id
...
def wait_until_page_changed(page, before_first_company_id: str):
    try:
        page.wait_for_function(
            """
            (beforeId) => {
            ...
            """,
            arg=before_first_company_id,
            timeout=30000
        )
        print("페이지 row 변경 확인")
    except Exception as e:
        print("페이지 row 변경 확인 실패:", e)
...
def go_to_page_2(page):
    print("2페이지 이동 시도")
    debug_screenshot(page, "19_before_go_to_page_2.png")

    before_first_company_id = get_first_company_row_id(page)
    print("이동 전 첫 row id:", before_first_company_id)

    try:
        page_2_button = page.locator("#paging li").filter(has_text="2").first

        if page_2_button.count() > 0:
            print("페이지 번호 2 클릭")
            page_2_button.click(timeout=10000)
            page.wait_for_timeout(5000)

            wait_until_page_changed(page, before_first_company_id)
            debug_screenshot(page, "20_after_go_to_page_2_by_number.png")

            print("2페이지 이동 완료")
            return

        print("페이지 번호 2 버튼을 찾지 못함")

    except Exception as e:
        print("페이지 번호 2 클릭 실패:", e)

    try:
        print("오른쪽 화살표 클릭 재시도")
        ...
        if next_button.count() == 0:
            debug_screenshot(page, "20_next_button_not_found.png")
            raise RuntimeError("오른쪽 화살표 버튼을 찾지 못했습니다.")

        next_button.click(timeout=10000)
        page.wait_for_timeout(5000)

        wait_until_page_changed(page, before_first_company_id)
        debug_screenshot(page, "20_after_go_to_page_2_by_arrow.png")

        print("오른쪽 화살표 클릭 후 페이지 이동 완료")

    except Exception as e:
        debug_screenshot(page, "20_go_to_page_2_failed.png")
        raise RuntimeError(f"2페이지 이동 실패: {e}")
...
def main():
    ...
    with sync_playwright() as p:
        ...
        print("1페이지 다운로드 테스트 시작")
        download_files_on_current_page(active_page, page_no="1")
        print("1페이지 다운로드 테스트 종료")

        print("2페이지 이동 테스트 시작")
        go_to_page_2(active_page)
        print("2페이지 이동 테스트 종료")

        print("2페이지 다운로드 테스트 시작")
        download_files_on_current_page(active_page, page_no="2")
        print("2페이지 다운로드 테스트 종료")
